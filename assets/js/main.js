(function($) {
	...
	(function() {
		...
		s.async = true;
		s.src = 'https://www.googletagmanager.com/gtag/js?id=G-DBM67G6JRH';
	...
	var root = document.body.dataset.root !== undefined ? document.body.dataset.root : '';
	...
	var NAV_ITEMS = {
		...
		cs: [
			...
			{ href: 'about.html',    label: 'O nás' },
			{ href: 'articles.html', label: 'Články' }
		]
	};
	...
	function injectNav() {
		...
		if (!ul) return;
		var items = NAV_ITEMS[lang] || NAV_ITEMS['en'];
		ul.innerHTML = items
			.map(function(i) { return '<li><a href="' + root + i.href + '">' + i.label + '</a></li>'; })
			.join('');
	}
	...
	function injectFooter() {
		...
		footer.innerHTML =
			'<ul class="icons">' +
				'<li><a href="https://www.instagram.com/badatales/" target="_blank" class="icon brands fa-instagram"><span class="label">Instagram</span></a></li>' +
				'<li><a href="https://www.tiktok.com/@badatales" target="_blank" class="icon brands fa-tiktok"><span class="label">TikTok</span></a></li>' +
				'<li><a href="https://www.youtube.com/@Badatales" target="_blank" class="icon brands fa-youtube"><span class="label">YouTube</span></a></li>' +
				'<li><a href="#" target="_blank" class="icon brands fa-facebook-f"><span class="label">Facebook</span></a></li>' +
				'<li><a href="mailto:talesofbada@gmail.com" onclick="copyEmail(event)" class="icon solid fa-envelope"><span class="label">Email</span></a></li>' +
			'</ul>' +
			'<ul class="copyright">' +
				'<li>&copy; Spectral by ajlkn. Edited by Badatales</li>' +
	...
	if ($body.hasClass('page-articles')) {
		...
		fetch('../articles.json')
			...
			.then(function(data) {
				...
				// Activate filter from URL param e.g. ?filter=scotland
				var urlFilter = new URLSearchParams(window.location.search).get('filter');
				if (urlFilter) {
					var btn = document.querySelector('[data-filter="' + urlFilter + '"]');
					if (btn) btn.click();
				} else {
					render();
				}
		...
		fetch('../articles.json')
			...
			.then(function(articles) {
				...
				if ($body.hasClass('is-mobile')) {
					// Mobile: activate card closest to screen center on scroll
					function activateClosestToCenter() {
					...
					window.addEventListener('scroll', activateClosestToCenter, { passive: true });
					activateClosestToCenter();

				} else {
					// Desktop: hover + auto-cycle
	...
	if ($body.hasClass('page-article')) {
		...
		var otherLang = lang === 'en' ? 'cs' : 'en';
		[lang, otherLang].forEach(function(l) {
			var link = document.createElement('link');
			link.rel      = 'alternate';
			link.hreflang = l;
			link.href     = 'https://badatales.com/' + l + '/articles/' + slug + '/';
		...
		var scrollReadEl = document.getElementById('scrollRead');
		if (scrollReadEl) scrollReadEl.textContent = strings.scrollRead;
		var youMightLikeEl = document.getElementById('youMightLikeHeading');
		if (youMightLikeEl) youMightLikeEl.textContent = strings.youMightLike;
		var moreArticlesEl = document.getElementById('moreArticlesBtn');
		if (moreArticlesEl) moreArticlesEl.textContent = strings.moreArticles;
